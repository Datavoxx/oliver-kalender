...
## Description
Rules and field definitions for the CREATE EVENT operation in the orchestrator. Use when modifying the create flow in `orchestrator.py`.
...
**MIDNIGHT RULE:** If end_time is "00:00" and start_time >= 20:00, set end_date = start_date + 1 day.
...
| `description` | 2-3 full sentences of context from subject |
...
| `recurrence` | null | Full RRULE string — set automatically if user mentions recurring pattern (see RECURRENCE section below) |

## Missing Field Questions (Swedish)
```python
{
  "subject":              "Vad gäller mötet/bokningen?",
  "date+start_time+end_time": "Vilket datum och vilken tid? (t.ex. 14/4 kl 10–12)",
}
```
Note: date, start_time, end_time are ALWAYS asked together in one question if any are missing.
...
event_body = {
    ...
    "reminders": {
        "useDefault": False,
        "overrides": [
            {"method": "popup", "minutes": fields.get("reminder_minutes", 120)},
...
`date` = first occurrence. The orchestrator adds `event_body["recurrence"] = [fields["recurrence"]]`.

Confirmation format when recurring:
```
{title} inbokat {recurrence_sv} kl {HH:MM}–{HH:MM} (första gången {weekday} {day}/{month}). Påminnelse {X} innan.
```
Example: `"Möte med Anna inbokat varje måndag kl 09:00–10:00 (första gången måndag 16/3). Påminnelse 30 minuter innan."`
...
```
{title} inbokat {weekday} {day}/{month} kl {HH:MM}–{HH:MM}. Påminnelse {X timmar/minuter} innan.
```
Example: `"Tandläkare inbokat fredag 15/1 kl 14:00–15:00. Påminnelse 2 timmar innan."`
...
## prompt.py
All create rules (required fields, MIDNIGHT RULE, reminder default, question wording) are implemented in `lib/prompt.py` under `=== STEP 2: COLLECT FIELDS — CREATE ===`.
...
PASS: "hej" (active intent=create in progress)
...
PASS: "boka ett event" (generic intent phrase, no subject)
      → intent=create, ready=false, reply asks "Vad handlar eventet om?"
```
